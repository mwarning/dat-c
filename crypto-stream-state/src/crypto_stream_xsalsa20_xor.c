                                  ...
                                  unsigned long long mlen)
{
  ...
  if (state->remainder) {
    ...
    while (rem < crypto_stream_xsalsa20_BLOCKBYTES && offset < mlen) {
    ...
    m += offset;
    mlen -= offset;
    state->remainder = rem % crypto_stream_xsalsa20_BLOCKBYTES; // This should never wrap, but always <= 64

    if (!mlen) return 0;
  }


  state->remainder = mlen & (crypto_stream_xsalsa20_BLOCKBYTES - 1);
  mlen -= state->remainder;

  crypto_stream_xsalsa20_xor_ic(c, m, mlen, state->nonce, state->block_counter, state->key);

  state->block_counter += mlen / crypto_stream_xsalsa20_BLOCKBYTES;

  if (state->remainder) {
    sodium_memzero(state->next_block + state->remainder, crypto_stream_xsalsa20_BLOCKBYTES - state->remainder);
    memcpy(state->next_block, m + mlen, state->remainder);

    crypto_stream_xsalsa20_xor_ic(state->next_block, state->next_block, crypto_stream_xsalsa20_BLOCKBYTES, state->nonce, state->block_counter, state->key);
